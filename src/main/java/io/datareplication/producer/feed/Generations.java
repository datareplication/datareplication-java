 ...
 * {@link FeedPageMetadataRepository.PageMetadata#generation()} is a little field with one very specific role: we need to
 * be able to determine which page is the "latest" page, even for the very short timeframe when we have two pages that
 * qualify.
 * <p>
 ...
 * <p>
 * To avoid rollover, we check the generation on the "latest" page before generating a batch of pages, and if it's reached
 * a limit (see {@link #MAX_GENERATION}) we reset it and save the page during a time when we only have one page without
 * next link (see {@link GenerationRotationService#rotateGenerationIfNecessary(Optional)}).
 * <p>
